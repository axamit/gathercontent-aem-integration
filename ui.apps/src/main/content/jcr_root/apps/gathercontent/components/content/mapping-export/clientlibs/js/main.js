...
$(function () {
    ...
    function fetchMappingData() {
        ...
        data.abstractTemplateLimitPath = $("#ext-abstract-template-limit-path").val().trim();
    ...
    function reloadMapping() {
        ...
        path = path.substring(0, path.lastIndexOf("/")) + ".ajax-export.html";
        var data = {};
        data.templatePath = $("#template-path").val().trim();
        data.abstractTemplateLimitPath = $("#ext-abstract-template-limit-path").val().trim();
    ...
    $(function () {
        ...
        $(".saveMapping").click(function () {
            ...
            $.ajax({
                ...
                success: function (responseData, textStatus, xhr) {
                    ...
                    window.location.replace(path.substring(0, path.lastIndexOf("/jcr:content")) + ".mappingexport.html");
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log("error");
                }
        ...
        $(".cancelEditOfMapping").click(function () {
            window.location = GCStringUtils.getExportMappingListUrl();
        });
        ...
        $("#moveTo-mapping-modal").dialog({
            ...
            buttons: [{
                ...
                click: function () {
                    ...
                    allSelectsOfTab.val('');
                    allSelectsOfTab.trigger('change');
                    ...
                    allSelectsOfNewTab.trigger('change');
            ...
            open: function (event, ui) {
                ...
                for (i = 0; i < allLabelsOfNewTab.length; i++) {
                    var lab = allLabelsOfNewTab.eq(i).text();
                    for (j = 0; j < data.length; j++) {
                        if (!GCStringUtils.isEmpty(lab) && data[j][0] === lab && data[j][1] !== null) {
            ...
            items: [
                {
                    cls: "required",
                    id: "mapping-name",
                    fieldDescription: "Add a mapping name. This will be used to distinguish mappings on export dialogs.",
                ...
                },
                {
                    cls: "required path",
                    id: "template-path",
                    fieldDescription: "Select template path. This page and all pages with the same template will be used as a base model for your field mappings.",
                ...
                {
                    cls: "required path",
                    id: "ext-abstract-template-limit-path",
                    fieldDescription: "Select content path which will be the starting point for the template pages search.",
                    fieldLabel: "Starting point for templates search",
                    rootPath: "/content",
                    xtype: "pathfield",
                    labelStyle: 'margin-top:-5px;height:24px;',
                    value: $('#mapper-abstract-template-limit-path').data('value') || "/content",
                    "listeners": {
                        "dialogselect": function (pathField, path) {
                            GATHERCONTENT.pathFieldEmptyDialogSelectCheck(pathField, path);
                            reloadMapping();
